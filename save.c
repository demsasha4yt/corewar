...
int one_argument(t_asm *asm_ms, char *str, t_token *current)///проверка %0 - T_DIR с числом
{
	...
	if (str[var.i] && str[var.i] == DIRECT_CHAR)
		var.i++;
	else if(str[var.i] && str[var.i] == LABEL_CHARS[17] && _is_number_char(str[var.i + 1])) ///если регистр 'r'
	{
		var.i++;
		_is_number_char(str[var.i]) ? 0 : asm_error(14, current->current_line, asm_ms);
		while (str[var.i] && _is_number_char(str[var.i]))
			var.i++;
		var.j = var.i;
		is_spacei(str, &var.i);
		if (str[var.i] != COMMENT_CHAR && str[var.i] != ALT_COMMENT_CHAR && str[var.i])
			asm_error(14, current->current_line, asm_ms);
		_save_info(asm_ms, str, current, var.j);
		return (0);
	}
	else
		asm_error(14, current->current_line, asm_ms);
	...
	{
		var.i++;
		while (str[var.i] && _is_label_char(str[var.i]))
			var.i++;
		var.j = var.i;
		is_spacei(str, &var.i);
		if (str[var.i] != COMMENT_CHAR && str[var.i] != ALT_COMMENT_CHAR && str[var.i])
	...
	else if ((str[var.i] == '-' || _is_number_char(str[var.i])) && str[var.i])
	{
		str[var.i] == '-' && ((!str[var.i + 1] || !_is_number_char (str[var.i + 1]))) ? asm_error(14, current->current_line, asm_ms) : var.i++;
		while (str[var.i] && _is_number_char(str[var.i]))
			var.i++;
		var.j = var.i;
		is_spacei(str, &var.i);
