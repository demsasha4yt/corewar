...
/*   Updated: 2020/02/10 17:40:00 by kturnips         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (er_nu == 26)
		ft_printf("%s:%d \x1B[31mCommand before name/comment
...
void	asm_error_p3(int er_nu, int line, t_asm *asm_ms)
{
	if (er_nu == 18)
		...
		ft_printf("%s:%d \x1B[31mBad symbols after
				comment declaration\n\033[0m", asm_ms->error_name, line);
	else if (er_nu == 21)
		ft_printf("%s:%d \x1B[31mBad character.
				No quotation mark met after name declaration\n\033[0m",
				asm_ms->error_name, line);
	else if (er_nu == 22)
		ft_printf("%s:%d \x1B[31mBad character. No quotation mark met after
				comment declaration\n\033[0m", asm_ms->error_name, line);
	else if (er_nu == 23)
